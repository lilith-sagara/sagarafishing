...
const RODS = [
    ...
    { tier: 36, name: '⚡ Lux Aeterna (Cahaya Abadi)', price: 0, max_tier: 11, cooldown: 0.5, luck_bonus: 1500, description: 'Pancingan suci dari keadilan ilahi.', tier_label: 'DIVINUS' },
...
function getMainMenuKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('🎣 Mancing', 'fish'), Markup.button.callback('🎒 Inventory', 'inventory')],
        [Markup.button.callback('🏪 Toko', 'shop'), Markup.button.callback('🎰 Gacha', 'gacha')],
        [Markup.button.callback('🏆 Leaderboard', 'leaderboard')]
    ]);
}

function getFishingResultKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('🎣 Mancing Lagi', 'fish'), Markup.button.callback('💰 Jual Semua', 'sell_all')]
    ]);
}
