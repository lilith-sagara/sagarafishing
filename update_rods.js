...
// We need to update RODS cooldowns according to user rules:
// - Tier 1 - 7: cooldown 5 - 3.5 seconds
// - Tier 7 - 15: cooldown 3 - 2.8 seconds (or smoothly descending)
// - Above Tier 15: 2 seconds (or 1-0 seconds for highest)

// Let's rewrite keyboards.js RODS completely with exact balanced cooldowns and correct fields.
const newRodsCode = `const { Markup } = require('telegraf');
...
// Replace RODS array in keyboards.js
const startRod = content.indexOf('const RODS = [');
const endRod = content.indexOf('function getMainMenuKeyboard');
if (startRod !== -1 && endRod !== -1) {
    const updated = newRodsCode + '\n' + content.slice(endRod);
    fs.writeFileSync('/home/lilith/sagarafishing/keyboards.js', updated);
    console.log('✅ Keyboards.js RODS successfully updated!');
} else {
    console.log('❌ Failed to find boundaries in keyboards.js');
}
